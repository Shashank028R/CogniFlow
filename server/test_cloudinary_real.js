...
fs.writeFileSync("real_test.pdf", validPDF);

cloudinary.uploader.upload("real_test.pdf", { resource_type: "raw" })
  .then(res => {
    console.log("RAW UPLOAD SUCCESS:");
    console.log("format:", res.format);
    console.log("resource_type:", res.resource_type);
    console.log("secure_url:", res.secure_url);
  })
  .catch(err => console.error("RAW ERROR:", err));
